...
"""
	This implementation of Music Service Song Lookup for YouTube Music is being deprecated.

	It, for some reason, doesn't play nice at all with YouTube's API, and keeps
	thinking we're bot traffic.

	It'll stay around for a bit, so we can roll back easily to this code if needed,
	and to reference to this in case the new song.py file isn't playing nice with something.

	TL;DR - This is old, use only for compatibility reasons where applicable.
"""
