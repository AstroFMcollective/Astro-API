...
async def search_query(query: str, filter_for_best_match: bool = True, media_types: list = None, is_explicit: bool = None, country_code: str = 'us'):
	...
	try:
		if media_types == None:
			query_media = ['song', 'musicTrack', 'album']
		else:
			am_types = []
			if 'song' in media_types:
				am_types.append('song')
			if 'music_video' in media_types:
				am_types.append('musicVideo')
			if 'collection' in media_types:
				am_types.append('album')
			query_media = am_types
		...
		for media_type in query_media:
			if media_type == 'song':
				...
				songs = await create_music_video_objects(
					json_response = results[query_media.index(media_type)],
					request = request,
					start_time = start_time,
					http_code = 200
