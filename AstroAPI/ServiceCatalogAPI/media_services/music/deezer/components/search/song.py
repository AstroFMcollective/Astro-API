...
async def search_song(artists: list, title: str, song_type: str = None, collection: str = None, is_explicit: bool = None, country_code: str = 'us', advanced_data_lookup: bool = True) -> object:
	...
	# If sinister things happen
		...
		await log(error, [discord.File(fp = StringIO(json.dumps(lookup_json, indent = 4)), filename = f'{title}.json'), discord.File(fp = StringIO(json.dumps(lookup_json, indent = 4)), filename = f'song.json')])
		return error